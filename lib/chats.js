...
module.exports.sendNewMessage = async (_id, chatId, newMessage, isChatMute, friends, isGroup) => {
  ...
  const { firstName, lastName, image, chats: userChats } = users.get(_id);
  let chatIndex;
...
module.exports.getUserComposeList = (_id) => {
  ...
  const composeList = chats.map((chat) => {
  ...
  if (!userMessages) {
    return User.findOne({ _id })
      ...
      .then(({ chats }) => {
        ...
        const currentExChat = userChats.find((chat) => chat.chatId === chatId);
