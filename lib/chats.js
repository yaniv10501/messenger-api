...
module.exports.getUserChatMessages = (_id, chatId) => {
  const { messages } = users.get(_id);
  const { messages: chatMessages } = messages.get(chatId);
  return chatMessages;
};

module.exports.sendNewMessage = async (_id, chatId, newMessage, isChatMute, friends) => {
  ...
  friends.forEach((friend) => {
    ...
    const addFriendMessage = (friendChatId) => {
      const friendMessages = this.getUserChatMessages(friendId, chatId);
    ...
    usersPromises.push(
      User.findOne({ _id: friendId })
        ...
        .then(async ({ chats }) => {
          ...
          console.log(chatId, friendUnreadCount);
          const bulkUpdate = [
            {
              updateOne: {
                filter: { _id },
          ...
          const currentMessages = this.getUserChatMessages(_id, chatId);
...
module.exports.setNewUserGroup = async (_id, groupName, groupFriends, image) => {
  const { firstName, lastName, birthday, gender, image: userImage, emptyGroup } = users.get(_id);
  ...
  groupFriendsList.forEach((friend, index) => {
    const friendFriendsIds = friednsIds.map((friendId, friendIndex) =>
      friendIndex === index ? { _id } : friendId
    );
    chatsPromises.push(
      User.findOneAndUpdate(
        {
          _id: friednsIds[index]._id,
        },
        {
          $push: {
            chats: {
  ...
  await Promise.all(chatsPromises).then((result) => console.log(result));
  console.log(newGroup);
};

module.exports.getUserChats = (_id) => {
  const { chats, loadedChats, chatsCount } = users.get(_id);
  console.log(loadedChats, chatsCount);
