...
module.exports.sendNewMessage = async (_id, chatId, newMessage, isChatMute, friends, isGroup) => {
  ...
  friends.forEach((friend, index) => {
    ...
    setNewUserNotif(friendId, {
      notifType: NEW_MESSAGE,
      otherUser: _id,
      actionId: chatId,
      utilId: newMessage._id,
    });
    const addFriendMessage = async (friendChatId) => {
      let { messages: friendMessages } = this.getUserChatMessages(friendId, chatId, { fast: true });
    ...
    usersPromises.push(
      User.findOne({ _id: friendId })
        ...
        .then(async ({ chats }) => {
          ...
          if (!isGroup) {
            let { messages: currentMessages } = this.getUserChatMessages(_id, chatId, {
              fast: true,
            });
    ...
    usersPromises.push(
      User.findOne({ _id })
        ...
        .then(async ({ chats }) => {
          ...
          let { messages: currentMessages } = this.getUserChatMessages(_id, chatId, { fast: true });
          if (currentMessages instanceof Promise) {
            currentMessages = await this.getUserChatMessages(_id, chatId, { fast: true })
              .then(({ messages: chatMessages }) => chatMessages)
              .catch((error) => {
