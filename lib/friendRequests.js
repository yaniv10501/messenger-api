...
module.exports.addUserFriendRequest = async (_id, friendId, index) => {
  try {
    ...
    if (friendRequest.otherUserId === friendId) {
      ...
      users.set(_id, { moreFriends, friendRequests }, { isNew: false });
      let friendMoreIndex;
      ...
      const bulkUpdate = addFriend(_id, friendRequestId, itemTime, itemDay, itemDate, dateNow);
...
module.exports.setResponseFriendRequest = (_id, otherUserId, index) => {
  ...
  if (currentPendingRequest.requestId === otherUserId) {
    ...
    const { firstName, lastName, image, friendRequests } = currentFriend;
    ...
    currentUser.pendingFriendRequests.splice(index, 1);
    currentUser.friends.unshift({
      _id: friendId,
      friendId: otherUserId,
    });
    if (friends.length > 20) {
      currentUser.friends.splice(20, 1);
    }
    currentUser.composeList.unshift({
      chatId,
      friends: [
        {
          _id: friendId,
    ...
    currentFriend.chatsCount += 1;
    currentFriend.friendRequests.splice(userIndexAtFriend, 1);
    currentFriend.friends.unshift({
      _id,
    ...
    currentFriend.composeList.unshift({
      chatId,
      friends: [
        {
          _id,
    ...
    const bulkUpdate = responseFriendRequest(_id, friendId, chatId);
